...
void OlapChunkSource::update_runtime_filter_partition_pruning(RuntimeState* state) {
    ...
    const int64_t newly_pruned_count = pruner->prune_by_bloom_filters(*factory->get_runtime_bloom_filters(), state);
...
void OlapChunkSource::_init_counter(RuntimeState* state) {
    ...
    if (auto* pruner = _scan_node->runtime_filter_partition_pruner(); pruner != nullptr) {
        ...
        _rf_partitions_pruned_counter = ADD_COUNTER(profile, "RuntimeFilterPartitionsPruned", TUnit::UNIT);
