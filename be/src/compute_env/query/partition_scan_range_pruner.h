...
// FE supplies boundary shapes; entries with invalid literals are dropped.
...
public:
    ...
    int64_t prune_by_in_filters(const std::vector<ExprContext*>& runtime_in_filters);
    // Per checkpoint: consume newly published bloom filters; a late one is retried.
    int64_t prune_by_bloom_filters(RuntimeFilterProbeCollector& runtime_bloom_filters, RuntimeState* state);
...
private:
    ...
    // Requires _update_mutex; returns the number of newly pruned partitions.
    int64_t publish_pruned_partitions(const PrunedPartitions& current, const PrunedPartitions& newly_pruned_partitions);
